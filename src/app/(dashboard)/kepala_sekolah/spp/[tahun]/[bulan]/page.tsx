...
export default async function DetailSppKepsekPage({ params }: { params: Promise<{ tahun: string, bulan: string }> }) {
  ...
  // Ensure it's a number. value is JSONB so it might be a string inside or just number.
  let sppNominal = 0
  if (settingData) {
  ...
  pembayarans?.forEach(p => {
    // Supabase returns related table as array if not 1-to-1, bypass via `as any` later
    sppMap.set(p.siswa_id, p)
  })

  const mergedData = (siswas || []).map(s => ({
    ...s,
    kelas: s.kelas as any, // Bypass TS array inference
